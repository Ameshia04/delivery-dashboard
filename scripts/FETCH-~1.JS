...
async function searchAll(jql, fields, expand, cap = 500) {
  ...
  while (issues.length < cap) {
    const qs = `jql=${encodeURIComponent(jql)}&startAt=${startAt}&maxResults=${pageSize}&fields=${fields.join(",")}${expand ? `&expand=${expand.join(",")}` : ""}`;
    const res = await fetch(`${BASE_URL}/rest/api/3/search?${qs}`, {
      headers: { Authorization: AUTH, Accept: "application/json" },
    });
    if (!res.ok) throw new Error(`Jira search failed: ${res.status} ${await res.text()}`);
    const data = await res.json();
    issues.push(...data.issues);
    if (!data.issues.length || data.issues.length < pageSize || issues.length >= data.total) break;
    startAt += pageSize;
  }
  return issues;
}
